/* Programme "Wii Camera to Pijama" v1.0, Aganyte, April 2020.
   Modifs Makoto Novembre 2022 Version 2.1:
    ...
    ok - Switch de sélection pour désactiver le mode Joystick.
    ok - Switch de sélection pour désactiver le mode Reload.
    ok - Ajout d'un filtre pour réduire les tremblements du curseur (Sacha)
*/
...
#define SwitchReload          8   // Mode reload On/Off
#define SwitchSuspendMouse    9   // (Actif par défaut si non câblé) Désactivation Mode Souris (le HID est présent, mais les data ne sont plus envoyées à la souris
#define SwitchDisableJoystick 10  // désactive le HID joystick
#define SwitchSuspendStick    11  // (Actif par défaut si non câblé)désactive le stick analogique du joystick
unsigned int SuspendMouse = 0;
unsigned int SuspendStick = 0;
...
void setup()
{
  ...
  // Switch selecteurs
  pinMode(SwitchReload, INPUT_PULLUP); // Mode reload
  pinMode(SwitchSuspendMouse, INPUT_PULLUP); // (Actif par défaut si non câblé) Désactivation Mode Souris (le HID est présent, mais les data ne sont plus envoyées à la souris
}
...
void loop()
{
  ...
  //variable filtre
  int Xf, Yf, x1=127, x2=127, y1=127, y2=127;
  ...
  for (;;)
  {
    ...
    Yf = Yf/3;
        ...
        Serial.print("Xf : ");
    		Serial.print((int)Xf);
    		Serial.print(",  Yf : ");
    		Serial.println((int)Yf);

    if ( ((int)Xf == 0) & ((int)Yf == 0) )
    {
      Mouse.Actual_X = 1;
      Mouse.Actual_Y = 1;
    }
    ...
    {
      Mouse.Actual_X = map((int)Xf, 0, 255, 32767, 0);
      Mouse.Actual_Y = map((int)Yf, 0, 255, 0, 32767);
    }
    ...
    ModeReload = digitalRead(SwitchReload);
    ...
    int xAxis = map(Xf, 0, 255, 255, 0);  // pour inverser l'axe X
    int yAxis = Yf;
    ...
    SuspendStick = digitalRead(SwitchSuspendStick);
    if (SuspendStick == 1) {  // 1 par défaut)
